...
function switchTab(tabId) {
    ...
    // --- Auto-foco Inteligente y Acciones de Pestaña ---
    setTimeout(() => {
        if (tabId === 'items') {
            const searchInput = document.getElementById('main-bank-search');
            // if (searchInput) { searchInput.focus(); } // Autofoco desactivado
        } else if (tabId === 'calc') {
            const searchInput = document.getElementById('calc-search-input');
            if (searchInput) { searchInput.focus(); searchInput.select(); }
        } else if (tabId === 'db') {
            // NUEVO: Extracción automática y silenciosa al entrar a la Base de Datos
            autoExtractInsumosFromBank(true); 
        }
    }, 150);
}
...
// --- NUEVA FUNCIÓN: EXTRACCIÓN INTELIGENTE DEL BANCO ---
function autoExtractInsumosFromBank(silent = true) {
    let addedCount = 0;
    const types = ['materiales', 'mano_obra', 'equipos'];
    
    types.forEach(type => {
        // 1. Crear un mapa de los insumos que YA existen en la BD para búsqueda rápida
        const dbMap = new Map();
        appData.database[type].forEach(item => {
            const key = `${(item.desc || "").trim().toLowerCase()}|${(item.unit || "").trim().toLowerCase()}`;
            dbMap.set(key, item);
        });

        // 2. Escanear todo el Banco de Ítems
        appData.itemBank.forEach(apu => {
            if (apu[type] && Array.isArray(apu[type])) {
                apu[type].forEach(res => {
                    const key = `${(res.desc || "").trim().toLowerCase()}|${(res.unit || "").trim().toLowerCase()}`;
                    
                    // 3. Si el insumo del Banco NO está en la Base de Datos, lo agregamos
                    if (!dbMap.has(key) && res.desc.trim() !== "") {
                        const newItem = {
                            id: Date.now() + Math.floor(Math.random() * 100000) + addedCount,
                            code: "0", // Se reordenará automáticamente al renderizar
                            desc: res.desc.trim(),
                            unit: res.unit.trim(),
                            price: res.price || 0
                        };
                        appData.database[type].push(newItem);
                        dbMap.set(key, newItem); // Lo añadimos al mapa para no duplicarlo en el mismo ciclo
                        addedCount++;
                    }
                });
            }
    ...
    // 4. Guardar y notificar resultados
    if (addedCount > 0) {
        saveData();
        renderDBTables();
        if (!silent) showToast(`Búsqueda inteligente: Se extrajeron ${addedCount} insumos nuevos del Banco.`);
    } else {
        if (!silent) showToast("La Base de Datos ya contiene todos los insumos del Banco.");
    }
}

// --- IMPORTADOR EXCEL APU ---
// --- 1. PROCESADOR DE APUS (Lógica original intacta) ---
function processComplexAPUExcel(file) {
    if (!file) return;
...
// Conectores para APUs (Moderno y Fallback)
function openComplexAPUExcel() { 
    openExcelFileModern(processComplexAPUExcel, 'fileComplexAPU'); 
}
function handleFallbackComplexAPU(input) { 
    if (input.files.length > 0) processComplexAPUExcel(input.files[0]); 
    input.value = ''; 
}
...
// --- 2. PROCESADOR DE VOLÚMENES (Lógica original intacta) ---
function processBudgetExcel(file) {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = async function (e) {
        try {
...
// Conectores para Volúmenes (Moderno y Fallback)
function openBudgetExcel() { 
    openExcelFileModern(processBudgetExcel, 'fileImportBudget'); 
}
function handleFallbackBudgetExcel(input) { 
    if (input.files.length > 0) processBudgetExcel(input.files[0]); 
    input.value = ''; 
}
...
async function exportInsumosToExcel() {
    ...
    await saveExcelBuffer(buffer, `Insumos_${tipoTexto[currentInsumosType]}_${new Date().toISOString().slice(0, 10)}.xlsx`);
    showToast('Excel exportado correctamente');
}

async function saveExcelBuffer(buffer, filename) {
    const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    
    // Usamos el guardado moderno inteligente
    const success = await saveFileModern(blob, filename, 'Documento de Excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', '.xlsx');
    
    // Opcional: si quieres un toast aquí o si las funciones padre ya lo tienen.
    // if (success) showToast('Excel exportado correctamente');
}
...
async function exportB1ToExcel() {
    ...
    zip.generateAsync({ type: "blob" }).then(async function (content) {
        ...
        const fileName = `Reportes_SICOES_5DEC_${pName}_${dateStr}.zip`;
        
        // Aplicamos el guardado moderno también al ZIP
        const success = await saveFileModern(content, fileName, 'Archivo ZIP Comprimido', 'application/zip', '.zip');
        if (success) showToast("Descarga de reportes lista");
    });
}

// --- EXPORTACIÓN_GUARDAR ---
// --- MODERN FILE SYSTEM ACCESS API: GUARDAR ---
async function saveFileModern(blob, defaultName, description, mimeType, extension) {
    if ('showSaveFilePicker' in window) {
        try {
            // MEJORA UX MÓVIL: Quitamos la extensión del nombre sugerido.
            let nameWithoutExt = defaultName;
            if (defaultName.endsWith(extension)) {
                nameWithoutExt = defaultName.slice(0, -extension.length);
            }
            ...
            if (err.name !== 'AbortError') console.error("Error guardando:", err);
            return false; // Cancelado o error
        }
    } else {
        // Fallback clásico: 'defaultName' (con extensión)
        saveAs(blob, defaultName);
        return true;
    }
}

async function exportBudgetOnlySURE() {
    ...
    const blob = new Blob([JSON.stringify(budgetData)], { type: 'application/vnd.presure.sure' });
    const ahora = new Date(); const d = String(ahora.getDate()).padStart(2,'0'); const m = String(ahora.getMonth()+1).padStart(2,'0'); const y = ahora.getFullYear();
    
    const success = await saveFileModern(blob, `PG_${d}${m}${y}.sure`, 'Presupuesto presure', 'application/vnd.presure.sure', '.sure');
    if (success) showToast('Presupuesto guardado');
}

async function exportBankSURB() {
    const blob = new Blob([JSON.stringify(appData.itemBank)], { type: 'application/vnd.presure.surb' });
    const ahora = new Date(); const d = String(ahora.getDate()).padStart(2,'0'); const m = String(ahora.getMonth()+1).padStart(2,'0'); const y = ahora.getFullYear();
    
    const success = await saveFileModern(blob, `BD_${d}${m}${y}.surb`, 'Base de datos presure', 'application/vnd.presure.surb', '.surb');
    if (success) showToast('Banco exportado');
}



// --- MODERN FILE SYSTEM ACCESS API: ABRIR EXCEL ---
async function openExcelFileModern(processCallback, fallbackInputId) {
    if ('showOpenFilePicker' in window) {
        try {
            ...
            if (err.name !== 'AbortError') console.error("Error al abrir Excel:", err);
        }
    } else {
        document.getElementById(fallbackInputId).click(); // Fallback
    }
}

// --- LÓGICA DE PROCESAMIENTO DE ARCHIVOS (Reutilizable) ---
function processProjectFile(file) {
    if (!file.name.toLowerCase().endsWith('.sure')) { 
        showToast('Por favor, seleccione un archivo .sure'); 
        return; 
    }
    const reader = new FileReader();
    reader.onload = function (e) {
        try {
            ...
            // Preserve current database and bank: importing a .sure should not overwrite the app's DB
            const preservedDatabase = appData.database;
            const preservedBank = appData.itemBank;

            // Merge only the project-related pieces: settings, modules, activeModuleId, projectItems
            if (loadedData.settings) appData.settings = { ...appData.settings, ...loadedData.settings };
            if (Array.isArray(loadedData.modules) && loadedData.modules.length > 0) appData.modules = loadedData.modules;
            if (loadedData.activeModuleId) appData.activeModuleId = loadedData.activeModuleId;
            if (Array.isArray(loadedData.projectItems)) appData.projectItems = loadedData.projectItems;

            // Restore preserved DB and bank to avoid accidental overwrites
            appData.database = preservedDatabase || { materiales: [], mano_obra: [], equipos: [] };
            appData.itemBank = preservedBank || [];

            // Ensure minimal defaults
            if (!appData.activeModuleId && appData.modules.length > 0) appData.activeModuleId = appData.modules[0].id;

            // Persist the loaded project (but not the database/bank changes)
            saveData();
            renderB1(); renderBankList(); renderDBTables(); loadSettingsToUI();
            switchTab('b1');
            showToast("Proyecto cargado correctamente (BD preservada)");
        } catch (err) {
            console.error(err); 
            showToast("Error al leer el archivo. Verifique que sea válido (.sure).");
        }
    };
...
function processBankFile(file) {
    if (!file.name.toLowerCase().endsWith('.surb')) {
        showToast("Formato no válido. Selecciona un archivo .surb");
        return;
    }
    const reader = new FileReader();
    reader.onload = function (e) {
        try {
            ...
            if (Array.isArray(loaded)) {
                ...
                saveData(); renderBankList(); renderDBTables();
                showToast(`Importado: ${apuCount} APUs y ${insumosCount} insumos nuevos`);
            }
        } catch (e) {
            console.error(e); 
            showToast("Error al leer el archivo .surb");
        }
    };
    reader.readAsText(file);
}

// --- FALLBACKS PARA INPUT TRADICIONAL ---
function handleFallbackProjectLoad(input) {
    if (input.files.length > 0) processProjectFile(input.files[0]);
    input.value = '';
}

function handleFallbackBankLoad(input) {
    if (input.files.length > 0) processBankFile(input.files[0]);
    input.value = '';
}

// --- MODERN FILE SYSTEM ACCESS API: ABRIR ---
async function openProjectSURE() {
    if ('showOpenFilePicker' in window) {
        try {
            const [fileHandle] = await window.showOpenFilePicker({
                types: [{
                    description: 'Proyecto PresuRE',
                    accept: { 'application/vnd.presure.sure': ['.sure'] }
                }],
                excludeAcceptAllOption: true, // ¡La magia para ocultar "Todos los archivos"!
                multiple: false
            });
            const file = await fileHandle.getFile();
        ...
        document.getElementById('fileProject').click(); // Fallback
    }
}

async function openBankSURB() {
    if ('showOpenFilePicker' in window) {
        try {
            const [fileHandle] = await window.showOpenFilePicker({
                types: [{
                    description: 'Base de Datos PresuRE',
                    accept: { 'application/vnd.presure.surb': ['.surb'] }
                }],
                excludeAcceptAllOption: true,
                multiple: false
            });
        ...
        document.getElementById('fileBank').click(); // Fallback
    }
}

// --- 3. PROCESADOR DE INSUMOS DB (Lógica original intacta) ---
async function processDBExcel(file, type) {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = async function (e) {
        try {
...
// Conector Híbrido para Insumos
async function importDBFromExcel() {
    const type = document.getElementById('dbImportType').value;
    const fallbackInput = document.getElementById('dbFile');
    
    // Si el usuario ya arrastró/seleccionó un archivo en el input visible clásico:
    if (fallbackInput.files.length > 0) {
        processDBExcel(fallbackInput.files[0], type);
        fallbackInput.value = ''; // Limpiamos el input después de procesar
        return;
    }
    
    // Si no ha seleccionado nada en el input visible, disparamos el selector nativo moderno
    if ('showOpenFilePicker' in window) {
        try {
            const [fileHandle] = await window.showOpenFilePicker({
                types: [{ 
            ...
            const file = await fileHandle.getFile();
            processDBExcel(file, type);
        } catch (err) { 
            if (err.name !== 'AbortError') console.error("Error al abrir Excel:", err); 
        }
    } else {
        // Fallback final: Si el navegador no soporta la API moderna, forzamos clic en el input
        fallbackInput.click();
        fallbackInput.onchange = function() {
            if (this.files.length > 0) processDBExcel(this.files[0], type);
            this.value = ''; // Limpiar
            this.onchange = null; // Quitar listener para evitar bucles
        };
    }
}
...
async function generateExcelReportSICOES(type, projectName) {
    ...
    if (type === 'masivo') { 
        ...
        const buffer = await workbook.xlsx.writeBuffer(); 
        await saveExcelBuffer(buffer, `PROYECTO_${projectName}_COMPLETO.xlsx`); 
    }
    else if (type === 'pg') { 
        buildB1(); 
        const buffer = await workbook.xlsx.writeBuffer(); 
        await saveExcelBuffer(buffer, "B1.xlsx"); 
    }
    else if (type === 'apu') { 
        buildAPU(); 
        const buffer = await workbook.xlsx.writeBuffer(); 
        await saveExcelBuffer(buffer, "B2.xlsx"); 
    }
    else { 
        buildInsumos(type, type); 
        const buffer = await workbook.xlsx.writeBuffer(); 
        await saveExcelBuffer(buffer, `Insumos_${type}.xlsx`); 
    }
}
